const Board = () => {
  ...
  // Use conditional logic to set a variable to either 'Player O' or  'Player X'
	let nextplayer_message = '';
	if (player == 1) {
		nextplayer_message = 'Next Player: Player X';
	}
	else {
		nextplayer_message = 'Next Player: Player O';
	}
  ...
  return (
    <div className="game-board">
      ...
      <div id="info">
		<h1 id="turn">{nextplayer_message}</h1>
        <h1>{status}</h1>
      </div>
    </div>
  );
    ...
    <button
      ...
      onClick={() => {
		  if (!filled) {
			...
			console.log(`Square: ${id} ALREADY filled`);
		}
